...
const Artical = (item) => (
  <artical className="flex flex-col shadow my-4" key={item.id}>
    <a href={`/${item.slug}`} className="hover:opacity-75">
      <img
        src={
          item.image ? item.image : 'https://source.unsplash.com/collection/1346951/1000x500?sig=1'
        }
        alt={item.title}
      />
    </a>
    <div className="flex flex-col justify-start p-6">
      <Link to={`/${item.slug}`} className="text-blue-700 text-sm font-bold uppercase pb-4">
        {item.category}
      </Link>
      <Link
        to={`/${item.slug}`}
      ...
      <Link to={`/${item.slug}`} className="flex uppercase text-gray-800 hover:text-black">
